...
def main(pipe, promptdict, morph_rad_pair, lookuptable, fname, img_dir, seg_img_dir, save_brack_path, hdr_dir, save_exr_path):

    img_orig = cv2.imread(f'{img_dir}{fname}.png')
    hdr_im = cv2.imread(f'{hdr_dir}{fname}_resized_e.exr', cv2.IMREAD_UNCHANGED)
    hdr_im = cv2.cvtColor(hdr_im, cv2.COLOR_BGR2RGB)
    hdr_im = du.modify_exposure_hdr(hdr_im, stops=0)
    # --------------------------------------------- Temp image files
    ...
    seg_im = cv2.imread(f'{seg_img_dir}{fname}.png')[:, :, 0]
    ...
    # --------------------------------------------- Upscale 4x
    cv2.imwrite(f'{save_brack_path}{suffix_code}_brack_0.png', cv2.cvtColor(img_orig, cv2.COLOR_BGR2RGB))
    for fstop in range(predicted_brack):
        im_out = du.modify_exposure(image2merge, fstop)
        cv2.imwrite(f'{save_brack_path}{suffix_code}_brack_{int(predicted_brack - fstop)}.png',
                    cv2.cvtColor(im_out, cv2.COLOR_BGR2RGB))

    for fstop in range(2):
        im_out = du.modify_exposure(image2merge, int(-(fstop + 1)))
        cv2.imwrite(f'{save_brack_path}{suffix_code}_brack_{int(predicted_brack + fstop + 1)}.png',
                    cv2.cvtColor(im_out, cv2.COLOR_BGR2RGB))

    for brack in range(predicted_brack + 3):
        ldr_path = f"{save_brack_path}{suffix_code}_brack_{brack}.png"
        img_path_list.append(ldr_path)
        exp_t = 1 / (2 ** int(brack))
        exp_list.append(exp_t)

    hdr = buildHDRwithPILWithouEXIF(img_path_list, exp_list)
    hdr = np.float32(hdr)
    cv2.imwrite(f"{save_exr_path}{suffix_code}_CN_hybrid_debevec_francesco.exr", cv2.cvtColor(hdr, cv2.COLOR_BGR2RGB))
...
if __name__ == "__main__":
    ...
    out_dir = "/home/singh_a_WMGDS.WMG.WARWICK.AC.UK/wmg/itmo/AutomateDITMO/outputs/hal/MIT5Kv6/brackets/"
    exr_out_dir = "/home/singh_a_WMGDS.WMG.WARWICK.AC.UK/wmg/itmo/AutomateDITMO/outputs/hal/MIT5Kv6/exrs/"

    exr_dir = "/home/singh_a_WMGDS.WMG.WARWICK.AC.UK/wmg/itmo/AutomateDITMO/MIT5K_Resized/"
    ...
    filenames = [f.split('.')[0] for f in os.listdir(rgb_dir) if isfile(join(rgb_dir, f))]
    # filenames =['5008_HDR']
    failed_files = []
    ...
    for i, pdict in enumerate(dicts):
        ...
        for j, morph_pair in enumerate(morphology_radius_pairs):
            ...
            for k, fname in enumerate(filenames):
                ...
                try:
                    main(pipe, pdict, morph_pair, lookuptable, fname, rgb_dir, seg_dir, path, exr_dir, exr_out_dir)
